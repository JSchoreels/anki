...
impl Collection {
    ...
    fn compute_all_params(&mut self, req: &mut UpdateDeckConfigsRequest) -> Result<()> {
        ...
        let job_lanes = compute_all_params_job_lanes(jobs);
        let results = job_lanes
            .into_par_iter()
            .flat_map(|lane| {
                lane.jobs
                    .into_iter()
        ...
        for (idx, name, result) in results {
            match result {
                Ok(params) => {
                    ...
                    *selected_fsrs_params_mut(&mut req.configs[idx]) = params.params;
                }
        ...
        let progress_thread = thread::spawn(move || {
            ...
            while !finished {
                ...
                if let Err(_err) = anki_progress.update(false, |state| {
                    ...
                    for (((progress_index, progress), done), _job_idx) in progress_indexes
                        .iter()
                        .zip(progresses.iter())
                        .zip(done.iter())
                        .zip(0..)
...
mod test {
    ...
    fn compute_all_params_test_job(name: &str, reviews: usize) -> ComputeAllParamsJob {
        ComputeAllParamsJob {
            index: 0,
            progress_index: 0,
            name: name.to_string(),
    ...
    #[test]
    fn compute_all_params_job_lanes_start_largest_and_balance_reviews() {
        let lanes = compute_all_params_job_lanes_with_count(
            vec![
                compute_all_params_test_job("small", 40),
                compute_all_params_test_job("largest", 100),
                compute_all_params_test_job("large", 90),
                compute_all_params_test_job("medium", 50),
            ],
            2,
        );
        ...
        assert_eq!(lanes[0].jobs[0].name, "largest");
        assert_eq!(lanes[1].jobs[0].name, "large");
    }
