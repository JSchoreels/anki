...
def temporal_calibration_report(
    ...
    *,
    ...
    for tau_days in RECENCY_TAU_DAYS:
        ...
        models[f"rwkv_platt_recent_{tau_days}d"] = result
...
def _load_samples(
    ...
    *,
    ...
    try:
        if not _table_columns(connection, RWKV_CACHE_TABLE):
            raise ProbeError(f"missing RWKV cache table: {RWKV_CACHE_TABLE}")
        deck_ids = _matched_deck_ids(connection, deck_match)
        rows = _cached_review_rows(connection, deck_ids=deck_ids, limit=limit)
    finally:
        connection.close()
...
def _samples_from_rows(
    ...
    *,
    ...
    for review_id, card_id, ease, duration_millis, prediction in rows:
