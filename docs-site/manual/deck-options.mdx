...
### Same-Day Reviews Ignore Review Limit

When enabled, a card already answered during the current scheduler day may be
shown again after the maximum reviews/day limit has been reached. The repeated
answer does not use another review from that day's limit. This applies whether
FSRS uses its learning/relearning queues or skips them, and also applies to
RWKV-selected same-day reviews. Intraday learning and relearning cards already
bypass the review limit without this option.
